...
def plot_metric(ax, tag, ylabel, ref_step):
    ...
    # 🔹 Formato de decimales del eje Y
...
def save_combined_metrics():
    """Genera la figura combinada con las 4 métricas"""
    fig, axes = plt.subplots(1, 4, figsize=(18, 4))
    plt.subplots_adjust(wspace=0.3, top=0.80)

    for ax, (tag, ylabel, ref_step) in zip(axes, METRICS):
        plot_metric(ax, tag, ylabel, ref_step)

    # Leyenda global arriba centrada
    ...
    out_file = os.path.join(OUTPUT_DIR, "all_metrics_legend_top_600dpi.png")
...
def save_individual_metrics():
    """Genera una imagen separada para cada métrica"""
    for tag, ylabel, ref_step in METRICS:
        ...
        tag_name = tag.split("/")[-1]  # limpia el prefijo
